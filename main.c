...
int main(int argc, char const *argv[])
{
	...
	while(!sair)
	{
		...
		if(redraw&&al_is_event_queue_empty(eventos))
		{
			...
			for (int i = 0; i < 4; i++)
			{
				al_draw_bitmap(bloco, pecaempixel[i].c, pecaempixel[i].l, 0);
			}		
...
void gradetopixel(void)
{
	for (int i = 0; i < 4; i++)
	{
		pecaempixel[i].l = (peca[TIPO][i] /10)*20;
		pecaempixel[i].c = (peca[TIPO][i] %10)*20;
	}
}
int inicie(void)
{
